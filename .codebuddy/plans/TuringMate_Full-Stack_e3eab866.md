...
overview: 面向408计算机考研的AI 1对1私教全栈应用。Vue 3前端 + Python Agent后端（多模态LLM + RAG知识库 + Agent工具链），部署于腾讯云生态。前后端同步开发，实现核心闭环：拍照搜题→苏格拉底式引导讲解。
design:
  architecture:
    framework: vue
    component: tdesign
...
todos:
  - id: project-init
    content: 初始化前后端项目：Vite+Vue3+TS 前端 + FastAPI 后端骨架，配置依赖和 Docker 编排
    status: completed
  - id: layout-and-gateway
    content: 搭建前端布局框架 + 后端 LLM Gateway 多模型网关和 API 路由骨架
    status: completed
    dependencies:
      - project-init
  - id: home-and-search
    content: 实现首页、拍照搜题页 + 后端题目识别 Agent 和图片上传工具
    status: completed
    dependencies:
      - layout-and-gateway
  - id: guided-chat
    content: 实现引导式对话页 + 苏格拉底 Agent 状态机 + SSE 流式输出
    status: completed
    dependencies:
      - home-and-search
  - id: correction-and-diagnosis
    content: 实现手写批改页、薄弱点诊断页 + 批改/诊断 Agent + ECharts 雷达图
    status: completed
    dependencies:
      - layout-and-gateway
  - id: code-visualization
    content: 实现代码可视化页 + 后端代码执行沙箱工具 + 步骤播放器
    status: completed
    dependencies:
      - layout-and-gateway
  - id: rag-and-integration
    content: 构建 RAG 管线 + 知识图谱工具，端到端联调全流程
    status: completed
    dependencies:
      - guided-chat
...
TuringMate 是面向408计算机考研的AI 1对1私教 Web 应用，核心差异在于用苏格拉底式引导教学替代"直接给答案"，帮助考生真正掌握解题能力。整体架构为：多模态大模型 Agent + RAG 知识库 + Agent 工具链 + Vue 3 前端，部署于腾讯云生态。
...
- **状态管理**：Pinia
- **路由**：Vue Router 4
- **样式**：Tailwind CSS 3.4 + TDesign Vue Next
...
采用**前后端分离 + Agent 驱动**的全栈架构。前端 Vue 3 负责 UI 渲染和交互，通过 HTTP/SSE 与 Python 后端通信。后端以 FastAPI 为服务入口，LangGraph 编排多 Agent 协作，RAG 管线提供知识检索，工具链扩展 Agent 能力。前后端通过 OpenAPI 契约保持接口一致。
...
- **检索策略**：混合检索 = 向量相似度(0.7) + BM25关键词(0.3) → 重排序
- **四科知识图谱**：MVP 阶段用结构化 JSON，后续可迁移至 Neo4j

### 多模型网关设计

统一接口，模型可插拔切换。通过环境变量/配置文件指定默认模型，运行时按场景路由（图片理解走多模态、引导推理走文本模型），无需修改业务代码。接口包含 chat、chat_with_image、stream_chat 三种方法。
...
| `/api/chat/stream` | POST(SSE) | 流式引导对话 |
...
│   ├── src/
│   │   ├── api/                      # API 服务层
│   │   │   ├── index.ts              # [NEW] Axios 实例 + 拦截器
│   │   │   ├── question.ts           # [NEW] 题目相关 API
│   │   │   ├── chat.ts               # [NEW] 引导对话 API（含 SSE 流式）
│   │   │   ├── correction.ts         # [NEW] 手写批改 API
│   │   │   ├── diagnosis.ts          # [NEW] 薄弱点诊断 API
│   │   │   └── visualization.ts      # [NEW] 代码可视化 API
│   │   ├── assets/
│   │   │   └── styles/
│   │   │       └── global.css        # [NEW] 全局 CSS 变量 + Tailwind 主题
│   │   ├── components/
│   │   │   ├── layout/
│   │   │   │   ├── AppHeader.vue     # [NEW] 顶部导航栏
│   │   │   │   ├── AppSidebar.vue    # [NEW] 侧边栏导航
│   │   │   │   └── AppLayout.vue     # [NEW] 主布局容器
│   │   │   ├── chat/
│   │   │   │   ├── ChatBubble.vue    # [NEW] 对话气泡
│   │   │   │   ├── ChatInput.vue     # [NEW] 对话输入框
│   │   │   │   └── ChatPanel.vue     # [NEW] 对话面板
│   │   │   ├── question/
│   │   │   │   ├── QuestionCard.vue  # [NEW] 题目卡片
│   │   │   │   └── TopicTag.vue      # [NEW] 知识点标签
│   │   │   ├── upload/
│   │   │   │   └── ImageUploader.vue # [NEW] 图片上传组件
│   │   │   └── visualization/
│   │   │       ├── ArrayVisual.vue   # [NEW] 数组可视化
│   │   │       ├── TreeVisual.vue    # [NEW] 树结构可视化
│   │   │       └── CodeStepPlayer.vue # [NEW] 步骤播放器
│   │   ├── composables/
│   │   │   ├── useChat.ts            # [NEW] 对话逻辑（SSE 流式接收 + 消息管理）
│   │   │   ├── useUpload.ts          # [NEW] 图片上传逻辑
│   │   │   ├── useDiagnosis.ts       # [NEW] 诊断数据分析
│   │   │   └── useVisualization.ts   # [NEW] 可视化控制
│   │   ├── router/
│   │   │   └── index.ts              # [NEW] 路由配置
│   │   ├── stores/
│   │   │   ├── user.ts               # [NEW] 用户状态
│   │   │   ├── question.ts           # [NEW] 题目状态
│   │   │   ├── chat.ts               # [NEW] 对话状态
│   │   │   └── diagnosis.ts          # [NEW] 诊断状态
│   │   ├── types/
│   │   │   ├── question.ts           # [NEW] 题目类型
│   │   │   ├── chat.ts               # [NEW] 对话类型
│   │   │   ├── diagnosis.ts          # [NEW] 诊断类型
│   │   │   └── user.ts               # [NEW] 用户类型
│   │   ├── views/
│   │   │   ├── HomeView.vue          # [NEW] 首页
│   │   │   ├── PhotoSearchView.vue   # [NEW] 拍照搜题
│   │   │   ├── GuidedChatView.vue    # [NEW] 引导式对话
│   │   │   ├── CorrectionView.vue    # [NEW] 手写批改
│   │   │   ├── DiagnosisView.vue     # [NEW] 薄弱点诊断
│   │   │   └── CodeVisualView.vue    # [NEW] 代码可视化
│   │   ├── App.vue
│   │   └── main.ts
│   ├── index.html
│   ├── package.json
...
│   ├── app/
│   │   ├── __init__.py
│   │   ├── main.py                   # [NEW] FastAPI 入口 + CORS + 生命周期
│   │   ├── config.py                 # [NEW] 配置管理（Pydantic Settings）
│   │   ├── api/                      # [NEW] API 路由层
│   │   │   ├── __init__.py
│   │   │   ├── deps.py              # [NEW] 依赖注入
│   │   │   └── v1/
│   │   │       ├── __init__.py
│   │   │       ├── question.py      # [NEW] 题目识别路由
│   │   │       ├── chat.py          # [NEW] 引导对话路由（含 SSE）
│   │   │       ├── correction.py    # [NEW] 手写批改路由
│   │   │       ├── diagnosis.py     # [NEW] 薄弱点诊断路由
│   │   │       └── visualization.py # [NEW] 代码可视化路由
│   │   ├── agents/                   # [NEW] Agent 模块
│   │   │   ├── __init__.py
│   │   │   ├── orchestrator.py      # [NEW] 主调度 Agent（LangGraph StateGraph）
│   │   │   ├── question_parser.py   # [NEW] 题目识别 Agent
│   │   │   ├── socratic_tutor.py    # [NEW] 苏格拉底引导 Agent（状态机）
│   │   │   ├── corrector.py         # [NEW] 批改 Agent
│   │   │   └── diagnostician.py     # [NEW] 诊断 Agent
│   │   ├── core/                     # [NEW] 核心模块
│   │   │   ├── __init__.py
│   │   │   ├── llm_gateway.py       # [NEW] 多模型网关（统一接口 + 模型切换）
│   │   │   └── tools.py             # [NEW] Agent 工具注册基类
│   │   ├── rag/                      # [NEW] RAG 子系统
│   │   │   ├── __init__.py
│   │   │   ├── loader.py            # [NEW] 文档加载（PyMuPDF + 真题解析）
│   │   │   ├── splitter.py          # [NEW] 文本分块策略
│   │   │   ├── embeddings.py        # [NEW] Embedding 模型抽象
│   │   │   ├── vectorstore.py       # [NEW] 向量存储抽象（Chroma/VectorDB）
│   │   │   └── retriever.py         # [NEW] 混合检索 + 重排序
│   │   ├── tools/                    # [NEW] Agent 工具实现
│   │   │   ├── __init__.py
│   │   │   ├── image_ocr.py         # [NEW] 图片识别工具
│   │   │   ├── question_search.py   # [NEW] 题库检索工具
│   │   │   ├── code_executor.py     # [NEW] 代码执行工具（沙箱）
│   │   │   └── knowledge_graph.py    # [NEW] 知识图谱查询工具
...
│   │   │   ├── chat.py
│   │   │   └── diagnosis.py
│   │   ├── schemas/                  # [NEW] Pydantic 请求/响应 Schema
│   │   │   ├── __init__.py
│   │   │   ├── question.py
│   │   │   ├── chat.py
│   │   │   ├── correction.py
...
│   └── .env.example
│
├── docker-compose.yml                # [NEW] 编排前后端 + Redis + MySQL
├── .gitignore
└── README.md
```
...
- **code-explorer**
- Purpose: 在搭建前后端项目结构时，搜索验证 Vue 3 + TDesign、LangGraph、FastAPI 的最佳实践和组件用法
- Expected outcome: 确保生成的代码符合 Vue 3 Composition API、TDesign 组件库、LangGraph Agent 框架的推荐模式

## Skill

- **pdf**
- Purpose: 在构建 RAG 知识库时，解析408教材 PDF 文件，提取文本和表格内容用于向量化入库
- Expected outcome: 将 PDF 教材内容结构化提取，为 RAG 管线提供高质量文档数据